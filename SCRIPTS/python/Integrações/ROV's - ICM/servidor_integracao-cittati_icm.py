...
connection_string = "DRIVER={/opt/microsoft/msodbcsql17/lib64/libmsodbcsql-17.10.so.4.1};SERVER=10.6.2.5;DATABASE="+database+";UID=hallsoft;PWD=hall%*tec"
#connection_string = "DRIVER={SQL Server};SERVER=10.6.2.5;DATABASE=producao_ho1;UID=hallsoft;PWD=hall%*tec"
...
print('Script Iniciado:',data_hora)

with engine.begin() as conn:
   
   # IMPEDIR QUE ELE EXECUTE NOVAMENTE ENQUANTO HOUVER ALGUM PROCESSO EM EXECUÇÃO
   ...
   if flag_insert != 2:
      try:
         with engine.begin() as conn:
            ...
            data_hora = dt.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
            print('Insert na temporária Finalizado:',data_hora)
         ...
         with engine.begin() as conn:
            ...
            print('Insert na RVV Finalizado..')
...
data_hora = dt.datetime.now().strftime("%d/%m/%Y %H:%M:%S")
print('Script Finalizado:',data_hora)
